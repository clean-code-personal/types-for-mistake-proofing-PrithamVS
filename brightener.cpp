...
shared_ptr<Image> IteratePixelsToBrighten(shared_ptr<Image> inputImage,int& attenuatedPixelCount, std::function<int(int,int)> getBrightness)
{        
        auto brightenedImage =
            make_shared<Image>(inputImage->m_rows, inputImage->m_columns,
                [inputImage, &attenuatedPixelCount, getBrightness](uint8_t* initPixels) {
                    for (int x = 0; x < inputImage->m_rows; x++) {
                        for (int y = 0; y < inputImage->m_columns; y++) {
                            int index = inputImage->pixelIndex(x, y);
                            initPixels[index] =
                                brightenPixel(inputImage->getPixel(x, y), getBrightness(x,y), attenuatedPixelCount);
                        }
                    }
                });
...
shared_ptr<Image> BrightenWholeImage(shared_ptr<Image> inputImage, int& attenuatedPixelCount) {
    ...
    return IteratePixelsToBrighten(
        inputImage,
        attenuatedPixelCount,
...
shared_ptr<Image> AddBrighteningImage(shared_ptr<Image> inputImage, shared_ptr<Image> imageToAdd,
    ...
    return IteratePixelsToBrighten(
        inputImage,
        attenuatedPixelCount,
        [imageToAdd](const int x, const int y) {
            return imageToAdd->getPixel(x, y);
        }
    );
}
